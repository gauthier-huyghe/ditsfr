 ...
 * Gère aussi les pièces jointes dites "embarquées"
...
class Mailer {
	...
	function Mailer($template_path = '')
	{
		...
		//
		// On récupére le domaine actuel dans le cas d'un dialogue SMTP
		...
		//
		// On récupére l'adresse IP pour l'en-tête abuse
		//
	...
	function recipients_list()
	{
		...
		else
		{
			...
			if( !isset($this->fix_bug_mail) )
			{
				...
				// Pas grave, on récupére le contenu du phpinfo et on le scan (Bwaahahaa ..)
				//
				else
				{
					ob_start();
			...
			//
			// Sendmail/qmail/[...] est utilisé, on renvoie le contenu de l'entète To et on le supprime
			//
			if( $this->fix_bug_mail == 1 )
			{
				$address = '';
			...
			else
			{
				...
				// pour les entètes Cc et Bcc si on utilise la fonction mail() et qu'un serveur
				// smtp est utilisé. On supprime donc la personnalisation des entêtes Cc et Bcc
				//
				// Dans le doute, on remplace également l'entète To (et donc, la personnalisation)
				//
	...
	/**
	 * Renvoie les entètes correspondant au type demandé
	 *
	 * @param string $type
	 *
	 * @access private
	...
	function compile_headers()
	{
		...
		//
		// La rfc2822 conseille de placer certains entètes dans un certain ordre
		//
